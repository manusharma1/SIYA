...
?>
 ...
 <div class="boxstyle1">
        ...
        <div class="boxstyle3">
			...
			<?php
			$meetingtypename = '';
			$columns = array('id','meetingtypeid','date','name');
			$sqlObj = new MainSQL();
			$conditions = array();
			$conditions['=']['isactive'] = '1';
			
			$sql = $sqlObj->SQLCreator('S', 'meetings', $columns, $conditions, '', '5', '');
			if($result = $sqlObj->FireSQL($sql)){
			...
			$meetingtext = $sqlObj->getCleanData($resultset->date).' - '.$sqlObj->getCleanData($resultset->name);
			$meetingtypeid = $resultset->meetingtypeid;
			$url='meetings/showMeeting/'.$id.'/';

			$columns2 = array('name');
			$conditions2 = array();
			$conditions2['=']['id'] = $meetingtypeid;
			
			$sql2 = $sqlObj->SQLCreator('S', 'meetingtypes', $columns2, $conditions2, '', '', '');
			if($result2 = $sqlObj->FireSQL($sql2)){
			if($sqlObj->getNumRows($result2) !=0){ // If Rows Exists
			if($resultset2 = $sqlObj->FetchResult($result2)){
			$meetingtypename = $resultset2->name;
			}
			}
			}
			?>
			...
			?>
			<a href="<?php echo MainSystem::URLCreator($url); ?>"><?php echo $meetingtext . '<br /> Type : '.$meetingtypename; ?></a>
			<br />
			<?php 
			echo $controls;
			?>
			...
			<?php
			if(PROJ_RUN_AJAX==1){
			$formaction2 = MainSystem::URLCreator('meetings/addMeeting/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
			}else{
			$formaction2 = MainSystem::URLCreator('meetings/addMeeting/');
			}
			?>

			<?php
			$returnarrayblockaddaccess = MainSystem::CheckModuleActionAccess('admin','meetings','addMeeting');
