...
?>
 ...
 <div class="boxstyle1">
		...
		<?php
		if(PROJ_RUN_AJAX==1){
		$formaction3 = MainSystem::URLCreator('whiteboard/addNewWhiteBoard/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
		}else{
		$formaction3 = MainSystem::URLCreator('whiteboard/addNewWhiteBoard/');
		}
		?>

		<?php
		$returnarrayblockaddaccess = MainSystem::CheckModuleActionAccess('admin','whiteboard','addNewWhiteBoard');
		...
		<form id="addform2" name="addform2" method="post" action="<?php echo $formaction3; ?>">
			...
			$sqlObj = new MainSQL();
			$sql = $sqlObj->SQLCreator('S', 'whiteboard', $columns3, $conditions3, '', '', '');
			...
			$url='whiteboard/showWhiteBoard/'.$id.'/';
			...
			$controlsarray['editblockcontent']['url'] = 'whiteboard/editWhiteBoard/'.$id.'/';
			...
			$controlsarray['deleteblockcontent']['url'] = 'whiteboard/deleteWhiteBoard/'.$id.'/';
			...
			$controlsarray['moveupblockcontent']['url'] = 'whiteboard/changeWhiteBoardOrder/up,'.$blockid.'/';
			}
			...
			$controlsarray['movedownblockcontent']['url'] = 'whiteboard/changeWhiteBoardOrder/down,'.$blockid.'/';
			}
			...
			if($returnarrayblockaddaccess['noerror'] == 1){
			?>
			<button type="submit">Add White Board</button>
			<?php
			}
			?>
