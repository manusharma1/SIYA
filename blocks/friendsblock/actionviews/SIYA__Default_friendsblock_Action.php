...
?>
 ...
 <div class="boxstyle1">
		...
		<?php
		$columns = array('id','friendid','status');
		$sqlObj = new MainSQL();
		$conditions = array();
		$conditions['=']['userid'] = $userid;
		$conditions['AND =']['isactive'] = '1';
		//$conditions['AND =']['status'] = 'REQUESTED'; it will work when enable  
		
		$sql = $sqlObj->SQLCreator('S', 'friends', $columns, $conditions, '', '5', '');
		...
		$url='friends/changeStatus/'.$id.'/';
		$userdetails = MainSystem::getUserDetailsByID($friendid);
		?>
		
		<p><?php echo $userdetails->fname.$userdetails->lname.'&nbsp;&nbsp;&nbsp;&nbsp; '.$status; ?></p>
		...
		$columns = array('id','userid','status');
		$sqlObj = new MainSQL();
		$conditions = array();
		$conditions['=']['friendid'] = $userid;
		$conditions['AND =']['isactive'] = '1';
		//$conditions['AND =']['status'] = 'REQUESTED'; it will work when enable  
		...
		while($resultset = $sqlObj->FetchResult($result)){
		$id = $sqlObj->getCleanData($resultset->id);
		$requestid = $sqlObj->getCleanData($resultset->userid);
		$status = $sqlObj->getCleanData($resultset->status);	
		$url='friends/changeStatus/'.$id.'/';
		$userdetails = MainSystem::getUserDetailsByID($requestid);

		?>
		
		<p><a href="<?php echo MainSystem::URLCreator($url); ?>"><?php echo $userdetails->fname.$userdetails->lname.'&nbsp;&nbsp;&nbsp;&nbsp; '.$status; ?></a>
