...
?>
 ...
 <div class="boxstyle1">
		...
		<?php
		if(PROJ_RUN_AJAX==1){
		$formaction2 = MainSystem::URLCreator('teachersdiary/addTeachersDiary/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
		}else{
		$formaction2 = MainSystem::URLCreator('teachersdiary/addTeachersDiary/');
		}
		?>

		<?php
		$returnarrayblockaddaccess = MainSystem::CheckModuleActionAccess('admin','teachersdiary','addTeachersDiary');
		if($returnarrayblockaddaccess['noerror'] == 1){
		?>

		<form id="addteachersdairyblock" name="addteachersdairyblock" method="post" action="<?php echo $formaction2; ?>">
		...
		?>
			...
			//$conditions['AND <=']['startdate'] = date('Y-m-d');
			//$conditions['AND >=']['enddate'] = date('Y-m-d');

			$sql = $sqlObj->SQLCreator('S', 'teachersdiary', $columns, $conditions, '', '0', '4');
			if($result = $sqlObj->FireSQL($sql)){
			...
			$controlsarray['editblockcontent']['url'] = 'teachersdiary/editTeachersDiary/'.$id.'/';
			...
			$controlsarray['deleteblockcontent']['url'] = 'teachersdiary/deleteTeachersDiary/'.$id.'/';
			...
			$controlsarray['moveupblockcontent']['url'] = 'holidays/changeHolidayOrder/up,'.$id.'/';
			}
			...
			$controlsarray['movedownblockcontent']['url'] = 'holidays/changeHolidayOrder/down,'.$id.'/';
			}
			...
			<a href="<?php echo MainSystem::URLCreator('teachersdiary/showTeachersDiary/'.$id.'/'); ?>"><?php echo $content_placeholder; ?></a>
			<br />
			<?php 
			echo $controls;
			?>
			...
			<button type="submit">Add Teachers Diary</button>
