<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('users/manageStudents/'.($id-1).'/');
	...
	$nextlinkurl =	MainSystem::URLCreator('users/manageStudents/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('users/studentsMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('users/studentsMultipleManage/');
	}


	$html_output .= MainSystem::CreateNoofRecordsDropDown($noofrecordstodisplay); 

	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','users','studentsMultipleManage');
	...
	$status_placeholder = MainSystem::URLCreator('users/changeStudentStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('users/editStudentRegistration/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('users/deleteStudentRegistration/'.$resultset->id.'/');
