<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('users/manageParents/'.($id-1).'/');
	...
	$sqlObj = new MainSQL();
	$conditions = array();
	$conditions['=']['entitytypetag'] = '@parent';
	$sql = $sqlObj->SQLCreator('S', 'users', $columns, $conditions,'', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('users/manageParents/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('users/parentMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('users/parentMultipleManage/');
	}
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','users','parentMultipleManage');
    ...
    $html_output .= '<td width="10%" ><a href="?orderby=name&orderbytype='.$orderbytype.'">
	<b>Name </b></td>
	...
	$conditions = array();
	$conditions['=']['entitytypetag'] = '@parent';
	$sql = $sqlObj->SQLCreator('S', 'users', $columns, $conditions,$orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	...
	$status_placeholder = MainSystem::URLCreator('users/changeParentStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('users/editParentRegistration/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('users/deleteParentRegistration/'.$resultset->id.'/');
