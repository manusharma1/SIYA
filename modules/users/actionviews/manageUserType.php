<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('users/manageUserType/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'usertypes', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('users/manageUserType/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('users/usersMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('users/usersMultipleManage/');
	}
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','users','usersMultipleManage');
    ...
    $html_output .= '<td width="10%" ><a href="?orderby=name&orderbytype='.$orderbytype.'"><b>User Type Tag</b></a></td>
	<td>Name</td>
	<td>Description</td>
	<td>Core</td>
	<td>Status</td>
	...
	// Get Page Data
	$columns = array('id','usertypetag','iscore','name','description','isactive');
	$sqlObj = new MainSQL();
	$conditions = array();
	$sql = $sqlObj->SQLCreator('S', 'usertypes', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	...
	$status_placeholder = ($resultset->iscore==1)?'#':MainSystem::URLCreator('users/changeUserTypeStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';
	 
	$edit_placeholder = ($resultset->iscore==1)?'#':MainSystem::URLCreator('users/editUserType/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->iscore==1)?'#':MainSystem::URLCreator('users/deleteUserType/'.$resultset->id.'/');
    ...
    $html_output .= '<td>'.$usertypetag_placeholder.'</td>   
    <td>'.$name_placeholder.'</td>
	<td>'.$description_placeholder.'</td>
	<td>'.$iscore_placeholder.'</td>';
 	if(MainSystem::CheckOtherUsersActionAccess('usertypes','addedby',$resultset->id)=='OK'){
