...
function receive(){
	$.getJSON("http://127.0.0.1/siya/modules/whiteboard/js/whiteboard.php?action=gpfp&value="+l_get,function(d){
		if(dd) c.closePath();
...
function transmit(p,color,line_width){
	...
	dstr = "http://127.0.0.1/siya/modules/whiteboard/js/whiteboard.php?action=tp&orid="+cur_id+"&value="+dstr+"&color="+color+"&lw="+line_width;
	$.getJSON(dstr,error);
}
