...
function receive(){
	$.getJSON("<?php echo PROJ_MODULES_DIR._WS.'whiteboard'._WS.'js'._WS.'whiteboard.php'; ?>?action=gpfp&value="+l_get,function(d){
		if(dd) c.closePath();
...
function transmit(p,color,line_width){
	...
	dstr = "<?php echo PROJ_MODULES_DIR._WS.'whiteboard'._WS.'js'._WS.'whiteboard.php'; ?>?action=tp&orid="+cur_id+"&value="+dstr+"&color="+color+"&lw="+line_width;
	$.getJSON(dstr,error);
}
