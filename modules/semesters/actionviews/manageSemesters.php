<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('semesters/manageSemesters/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'semesters', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('semesters/manageSemesters/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('semesters/semesterMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('semesters/semesterMultipleManage/');
	}	
	// Define placeholders
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','semesters','semesterMultipleManage');
    ...
    $html_output .= '<td width="10%"><a href="?orderby=fname&orderbytype='.$orderbytype.'"><b>Semestercode </b></td>
	...
	// Get Page Data
	$columns = array('id','semestercode','title','description','isactive');
	$sqlObj = new MainSQL();
	
	$sql = $sqlObj->SQLCreator('S', 'semesters', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) !=0){ // If Rows Exists
	
	while($resultset = $sqlObj->FetchResult($result)){

	$semestercode_placeholder = ($resultset->semestercode=='')?'':$sqlObj->getCleanData($resultset->semestercode);
	...
	$status_placeholder = MainSystem::URLCreator('semesters/changeSemesterStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('semesters/editSemesterRegistration/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('semesters/deleteSemesterRegistration/'.$resultset->id.'/');
    ...
    $html_output .= '<td>'.$semestercode_placeholder.'</td>
    <td>'.$title_placeholder.'</td>
	<td>'.$description_placeholder.'</td>';
 	if(MainSystem::CheckOtherUsersActionAccess('semesters','addedby',$resultset->id)=='OK'){
	$html_output .= '<td><a href="'.$status_placeholder.'">'.$status_text_placeholder.'</a></td>
    <td><a href="'.$edit_placeholder.'">'.$lang['siya']['semesters']['EDIT'] .' </a></td>
    <td><a href="'.$delete_placeholder.'">'.$lang['siya']['semesters']['DELETE'].' </a></td>';
	}else{
