<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('meetings/manageMeetingTypes/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'meetingtypes', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('meetings/manageMeetingTypes/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('meetings/meetingTypeMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('meetings/meetingTypeMultipleManage/');
	}	
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','meetings','meetingTypeMultipleManage');
    ...
    $html_output .= '<td width="10%"><a href="?orderby=fname&orderbytype='.$orderbytype.'"><B>Name</B></td>
	...
	$sql = $sqlObj->SQLCreator('S', 'meetingtypes', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	...
	$status_placeholder = MainSystem::URLCreator('meetings/changeMeetingTypeStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('meetings/editMeetingTypes/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('meetings/deleteMeetingTypes/'.$resultset->id.'/');
    ...
    <td>'.$description_placeholder.'</td>';
 	if(MainSystem::CheckOtherUsersActionAccess('meetingtypes','addedby',$resultset->id)=='OK'){
	$html_output .= '<td><a href="'.$status_placeholder.'">'.$status_text_placeholder.'</a></td>
    <td><a href="'.$edit_placeholder.'">'.$lang['siya']['meetings']['EDIT'] .' </a></td>
    <td><a href="'.$delete_placeholder.'">'.$lang['siya']['meetings']['DELETE'].' </a></td>';
	}else{
