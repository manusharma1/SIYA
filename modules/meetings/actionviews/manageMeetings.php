<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('meetings/manageMeetings/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'meetings', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('meetings/manageMeetings/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('meetings/meetingsMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('meetings/meetingsMultipleManage/');
	}	
    ...
    <td width="10%"><a href="?orderby=fname&orderbytype='.$orderbytype.'"><B>Name</B></td>
	...
	$sql = $sqlObj->SQLCreator('S', 'meetings', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) !=0){
	...
	$status_placeholder = MainSystem::URLCreator('meetings/changeMeetingStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('meetings/editMeeting/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('meetings/deleteMeeting/'.$resultset->id.'/');

	$tr_bg_color = ($tr_bg_value==0)?'evenclass':'oddclass';
	$tr_bg_value = ($tr_bg_value==0)?1:0;
	
	$html_output .= '<tr class="'.$tr_bg_color.'">
	<td width="5%" ><input type="checkbox" id="checkbox_'.$resultset->id.'" name="checkbox['.$resultset->id.']" /></td>
    <td>'.$name_placeholder.'</td>
    <td>'.$description_placeholder.'</td>
 	<td><a href="'.$status_placeholder.'">'.$status_text_placeholder.'</a></td>
    <td><a href="'.$edit_placeholder.'">Edit </a></td>
    <td><a href="'.$delete_placeholder.'">Delete </a></td>
	</ul>';
	} // while
