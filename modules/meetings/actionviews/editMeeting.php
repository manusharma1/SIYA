...
$columns = array('id','batchid','groupid','meetingtypeid','date','name','description');
$conditions = array();
$conditions['=']['id'] = $id;
$sqlObj = new MainSQL();
...
$meetingtypeid_tag = $sqlObj->getCleanData($resultset->meetingtypeid);
...
($resultsetmenu->id == $meetingtypeid_tag)?$htmlarray[]['option']['nonattribute'] = 'SELECTED':'';
$htmlarray[]['option']['close'] = '';
...
<?php
if(PROJ_RUN_AJAX==1){
$formaction = MainSystem::URLCreator('meetings/saveMeeting/'.$id.'/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
}else{
$formaction = MainSystem::URLCreator('meetings/saveMeeting/'.$id.'/');
}
?>
<form id="editmeetingform" name="editmeetingform" method="post" action="<?php echo $formaction; ?>">
