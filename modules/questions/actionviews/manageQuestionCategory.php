<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('questions/manageQuestionCategory/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'questionscategories', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('questions/manageQuestionCategory/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('questions/questionsCategoryMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('questions/questionsCategoryMultipleManage/');
	}	
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','questions','questionsCategoryMultipleManage');
	...
	$sql = $sqlObj->SQLCreator('S', 'questionscategories', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) !=0){
	...
	$status_placeholder = MainSystem::URLCreator('questions/changeQuestionCategoryStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('questions/editQuestionCategory/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('questions/deleteQuestionCategory/'.$resultset->id.'/');
    ...
    <td>'.$description_placeholder.'</td>';
 	if(MainSystem::CheckOtherUsersActionAccess('questionscategories','addedby',$resultset->id)=='OK'){
