<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('payments/managePaymentTypes/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'paymenttypes', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('payments/managePaymentTypes/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('payments/paymentTypesMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('payments/paymentTypesMultipleManage/');
	}	
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','payments','paymentsMultipleManage');
	...
	$sql = $sqlObj->SQLCreator('S', 'paymenttypes', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	...
	$status_placeholder = MainSystem::URLCreator('payments/changePaymentTypeStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('payments/editPaymentTypes/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('payments/deletePaymentTypes/'.$resultset->id.'/');
    ...
    <td>'.$description_placeholder.'</td>';
 	if(MainSystem::CheckOtherUsersActionAccess('paymenttypes','addedby',$resultset->id)=='OK'){
	$html_output .= '<td><a href="'.$status_placeholder.'">'.$status_text_placeholder.'</a></td>
    <td><a href="'.$edit_placeholder.'">'.$lang['siya']['payments']['EDIT'] .' </a></td>
    <td><a href="'.$delete_placeholder.'">'.$lang['siya']['payments']['DELETE'].' </a></td>';
	}else{
