<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('holidays/manageHolidays/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'holidays', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('holidays/manageHolidays/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('holidays/holidaysMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('holidays/holidaysMultipleManage/');
	}
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','holidays','holidaysMultipleManage');
    ...
    $html_output .= '<td width="10%" ><a href="?orderby=type&orderbytype='.$orderbytype.'"><b>Type</b></a></td>
	...
	// Get Page Data
	$columns = array('id','type','date','day','isactive');
	$sqlObj = new MainSQL();
	
	$sql = $sqlObj->SQLCreator('S', 'holidays', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) !=0){ // If Rows Exists
	
	while($resultset = $sqlObj->FetchResult($result)){

	$type_placeholder = ($resultset->type=='')?'&nbsp;':$sqlObj->getCleanData($resultset->type);
	$date_placeholder = ($resultset->date=='')?'&nbsp;':$sqlObj->getCleanData($resultset->date);
	$day_placeholder = ($resultset->day=='')?'&nbsp;':$sqlObj->getCleanData($resultset->day);

	$status_placeholder = MainSystem::URLCreator('holidays/changeHolidayStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('holidays/editHoliday/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('holidays/deleteHolidays/'.$resultset->id.'/');
	...
	$html_output .= '<td>'.$type_placeholder.'</td>
    <td>'.$date_placeholder.'</td>
	<td>'.$day_placeholder.'</td>';
 	if(MainSystem::CheckOtherUsersActionAccess('holidays','addedby',$resultset->id)=='OK'){
	
	$html_output .= '<td><a href="'.$status_placeholder.'">'.$status_text_placeholder.'</a></td>
    <td><a href="'.$edit_placeholder.'">'.$lang['siya']['holidays']['EDIT'] .' </a></td>
    <td><a href="'.$delete_placeholder.'">'.$lang['siya']['holidays']['DELETE'].' </a></td>';
	}else{
