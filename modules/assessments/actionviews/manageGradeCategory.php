<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('assessments/manageGradeCategory/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'gradecategories', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('assessments/manageGradeCategory/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('assessments/gradeCategoryMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('assessments/gradeCategoryMultipleManage/');
	}	
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','gradecategories','manageGrade');
    ...
    $html_output .= '<td width="10%"><a href="?orderby=fname&orderbytype='.$orderbytype.'">'.$lang['siya']['NAME'].'</td>
	<td width="10%" >'.$lang['siya']['DESCRIPTION'].'</td>
	<td width="5%" >'.$lang['siya']['STATUS'].'</td>
    <td width="5%" >'.$lang['siya']['EDIT'].'</td>
    <td width="5%" >'.$lang['siya']['DELETE'].'</td>
	</tr>';
	...
	$sql = $sqlObj->SQLCreator('S', 'gradecategories', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) !=0){ 
	...
	$status_placeholder = MainSystem::URLCreator('assessments/changeGradeCategoryStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('assessments/editGradeCategory/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('assessments/deleteGradeCategory/'.$resultset->id.'/');
    ...
    <td>'.$description_placeholder.'</td>';
 	if(MainSystem::CheckOtherUsersActionAccess('gradecategories','addedby',$resultset->id)=='OK'){
