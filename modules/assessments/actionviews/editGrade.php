...
$columns = array('id','gradecategoryid','assessmenttypeid','gradetype','startrange','endrange','marks','grade','gradepoint');
...
$sql = $sqlObj->SQLCreator('S', 'grades', $columns, $conditions, '', '', '');

if($result = $sqlObj->FireSQL($sql)){
if($sqlObj->getNumRows($result) !=0){ 
if($resultset = $sqlObj->FetchResult($result)){
$id = $sqlObj->getCleanData($resultset->id);
$gradecategoryid =  $sqlObj->getCleanData($resultset->gradecategoryid);
$assessmenttypeid_placeholder =  $sqlObj->getCleanData($resultset->assessmenttypeid);
$gradetype_placeholder =  $sqlObj->getCleanData($resultset->gradetype);
$startrange_placeholder =  $sqlObj->getCleanData($resultset->startrange);
$endrange_placeholder =  $sqlObj->getCleanData($resultset->endrange);
$marks_placeholder =  $sqlObj->getCleanData($resultset->marks);
$grade_placeholder =  $sqlObj->getCleanData($resultset->grade);
$gradepoint_placeholder =  $sqlObj->getCleanData($resultset->gradepoint);
}
}
}
...
$assessmenttypeid_placeholder_placeholder = $HTMLObj->HTMLCreator($htmlarray);
...
$htmlarray[]['option']['start'] = '';
$htmlarray[]['option']['value'] = $sqlObj->getCleanData($resultsetmenu->id);
($resultsetmenu->id == $gradecategoryid_placeholder)?$htmlarray[]['option']['nonattribute'] = 'SELECTED':'';
$htmlarray[]['option']['close'] = '';
...
<?php
if(PROJ_RUN_AJAX==1){
$formaction = MainSystem::URLCreator('assessments/saveGrade/'.$id.'/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
}else{
$formaction = MainSystem::URLCreator('assessments/saveGrade/'.$id.'/');
}
?>
<form id="editgradeform" name="editgradeform" method="post" action="<?php echo $formaction; ?>">

<fieldset>

	<legend><?php echo $lang['siya']['assessments']['EDIT_GRADE'];?></legend>
	...
	<ol>
		...
		<?php echo $assessmenttypeid_placeholder_placeholder; ?>
		</li>		
		...
		<input id="marks" name="marks" type="text" placeholder="<?php echo $lang['siya']['assessments']['ENTER_MARKS'];?>" value="<?php echo $marks_placeholder; ?>" <?php echo _FORM_FINAL; ?> />
		</li>
