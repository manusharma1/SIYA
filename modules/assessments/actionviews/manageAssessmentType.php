<?php
	...
	$previouslinkurl =	MainSystem::URLCreator('assessments/manageAssessmentType/'.($id-1).'/');
	...
	$columns = array('id');
	$sqlObj = new MainSQL();
	$sql = $sqlObj->SQLCreator('S', 'assessmenttypes', $columns, '', '', $startlimit2, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) > 0){
	$nextlinkurl =	MainSystem::URLCreator('assessments/manageAssessmentType/'.($id+1).'/');
	...
	if(PROJ_RUN_AJAX==1){
	$formaction = MainSystem::URLCreator('assessments/assessmentMultipleManage/','ajax','post','',PROJ_AJAX_DEFAULT_HTML_ID_FOR_TEMPLATE,false);
	}else{
	$formaction = MainSystem::URLCreator('assessments/assessmentMultipleManage/');
	}	
	...
	$returnarraymanageccess = MainSystem::CheckModuleActionAccess('admin','assessments','assessmentMultipleManage');
	...
	<td width="10%" ><B>'.$lang['siya']['DESCRIPTION'].'</B></td>
	...
	$sql = $sqlObj->SQLCreator('S', 'assessmenttypes', $columns, '', $orderby, $startlimit, $noofrecordstodisplay);
	if($result = $sqlObj->FireSQL($sql)){
	if($sqlObj->getNumRows($result) !=0){
	...
	$status_placeholder = MainSystem::URLCreator('assessments/changeAssessmentTypeStatus/'.$resultset->id.'/');
	$status_text_placeholder = ($resultset->isactive==0)?'Not Active':'Active';

	$edit_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('assessments/editAssessmentType/'.$resultset->id.'/');
	$delete_placeholder = ($resultset->id=='')?'#':MainSystem::URLCreator('assessments/deleteAssessmentType/'.$resultset->id.'/');
