...
if($type=='FITB'){
	...
	if($result = $sqlObj->FireSQL($sql)){
	$_SESSION['message'] = self::$lang['siya']['tests']['SAVE_USER_ANSWERS_SAVED'];
	
	//$returnid = ($id=='')?$sqlObj->getLastInsertID():$id;
	//MainSystem::URLForwarder(MainSystem::URLCreator('users/editUserType/'.$returnid.'/'));
	
	}else{
	$_SESSION['message'] = self::$lang['siya']['tests']['SAVE_USER_ANSWERS_CANNOT_BE_SAVED'];
	...
	$actionviewresult = MainSystem::CallOtherActionView(__CLASS__,'saveUserAnswers'); // REVIEW ABOUT THE SECURITY HERE//
	$functionreturnarray['title_placeholder'] = self::$lang['siya']['tests']['ERROR_SAVE_USER_ANSWERS_CANNOT_BE_SAVED'];
